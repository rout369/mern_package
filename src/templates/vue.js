...
async function generateVueTemplates(config) {
  ...
  console.log(chalk.gray(`🔍 Generating Vue.js-compatible boilerplate in ${clientPath}`));
...
${features.includes('auth') ? `async function checkAuth() {
  try {
    ...
    if (token) {
      ...
      document.getElementById('content').innerHTML = \`<p>Welcome, \${response.data.name}!</p><button onclick="logout()">Logout</button>\`;
    } else {
      document.getElementById('content').innerHTML = '<p>Please log in.</p><button onclick="showLogin()">Login</button>';
    }
  } catch (error) {
...
${features.includes('upload') ? `async function handleFileUpload(event) {
...
${database === 'postgresql' ? 'VITE_DB_URL=postgresql://user:password@localhost:5432/dbname' : ''}`;
      await fs.writeFile(path.join(clientPath, '.env'), envContent);
    }

    console.log(chalk.green(`✓ Vue.js-compatible boilerplate generated in ${clientPath}`));
  } catch (error) {
    console.error(chalk.red(`Failed to generate Vue.js boilerplate: ${error.message}`));
    throw error;
  }
}

module.exports = { generateVueTemplates };
